...
- **User deletion**: clients are hard-deleted; `users.is_active` and `users.deleted_at` were tried once and removed for all roles in favor of this. Foreign keys to users/clients were chosen so a client delete doesn't destroy history: `appointments.client_id` is `ON DELETE SET NULL`, `notifications_log.client_id` is `ON DELETE CASCADE` (notifications aren't worth keeping without their client), and `recommendation_history.client_id` is `CASCADE`. Barbers, managers, and receptionists are **soft-deleted** instead (decided and implemented): each of those three tables has its own `deleted_at timestamptz` (`NULL` = active), and "deleting" one is an `UPDATE`, never a `DELETE` — the `users`/role-extension row never goes away, so `appointments.barber_id`, `shops.manager_id`, and `supply_movements.performed_by` (all still `RESTRICT`/`NOT NULL`, never actually exercised on this path) keep pointing at a real, named row with no need for a name/role snapshot. `trg_barbers_check_deletable` blocks setting `barbers.deleted_at` while the barber has a `scheduled`/`checked_in` appointment (terminal statuses don't block it); `trg_managers_check_deletable` blocks setting `managers.deleted_at` while any shop still has them as `manager_id`. No guard exists for `receptionists.deleted_at` — nothing has an FK on `receptionists.user_id`. Known trade-off: `users.email`/`users.phone` stay globally `UNIQUE` and untouched by this, so a soft-deleted staff member's email/phone stays reserved indefinitely.
- **Movement authorship**: `supply_movements.performed_by` is `NOT NULL` and references `users(id)` with no `ON DELETE` action (defaults to `NO ACTION`, i.e. blocks deletion). This relies on the fact that only receptionists and managers ever perform a movement, and both are soft-deleted rather than hard-deleted (see "User deletion" above), so `performed_by` can never be orphaned — no name/role snapshot is needed the way `appointment_services.price_at_booking` needs one for prices.
...
- **CI/CD**: a single GitHub Actions workflow (`.github/workflows/cicd-package.yml`) with four jobs, gated on pushes/PRs to `main`/`dev`:
  1. **`test`** — spins up a real `postgres:18` service container, loads `backend/database/init_database.sql` into it, installs both `backend` and `frontend` dependencies, and runs the backend's Jest/Supertest suite against that database.
