...
- **Loyalty**: implemented as a running counter (`clients.completed_services_count`), not a ledger/transactions table. An `update_client_loyalty_count()` trigger fires when an appointment's status becomes `completed`: it increments the counter, or resets it to `0` if the appointment is flagged `is_reward_redemption`. The trigger is skipped when `client_id IS NULL` (guest appointments, or a deleted client's). There is no `loyalty_accounts`/`loyalty_transactions` table — redemption history for reporting would have to be derived from `appointments.is_reward_redemption` plus timestamps, since there's no separate ledger today.
...
- **Supply movements**: `supply_movements` is an append-only ledger of stock entries and exits (`movement_type` enum `in`/`out`, `quantity` always positive with the direction coming from the type, optional `unit_cost` and `reason`). It has no `updated_at` and no update trigger. It is **not** wired to `supply_stock`: nothing in the database updates `quantity_on_hand` when a movement is inserted, so the backend must write both in one transaction (or a trigger must be added later — undecided).
- **User deletion**: users are hard-deleted; `users.is_active` and `users.deleted_at` were removed, and `getUserByEmail` no longer filters on them. Foreign keys to users/clients were chosen so a delete doesn't destroy history: `appointments.client_id` is `ON DELETE SET NULL`, `notifications_log.client_id` is `ON DELETE CASCADE` (notifications aren't worth keeping without their client), `recommendation_history.client_id` is `CASCADE`, and `supply_movements.performed_by` is `SET NULL`. `appointments.barber_id` is still `RESTRICT`. The business rule is that a barber can't be deleted while they have pending appointments, which must be cancelled first (decided, **not implemented**). Note that `RESTRICT` is stricter than that rule: it also blocks deleting a barber with completed or cancelled appointments, so even after cancelling everything the deletion would still fail. See the open items in "Still open" below.
- **Movement authorship**: because `performed_by` becomes `NULL` when the user is deleted, `supply_movements` also stores `performed_by_name` (`first_name last_name`) and `performed_by_role` as snapshots. The `set_supply_movement_performer()` trigger (`BEFORE INSERT` only) fills them from `users`, overwriting any value the caller sends, and rejects rows with a `NULL` or nonexistent `performed_by` (movements are never system-generated). It doesn't fire on `UPDATE`, so the `SET NULL` from a user deletion leaves the snapshot intact. This mirrors the snapshot approach of `appointment_services.price_at_booking`. Trade-off: the name isn't a key, so movements of a deleted user can be found by `performed_by_name`, but homonyms can't be distinguished. On the backend, `types/entities/supplyMovement.interface.ts` exports `CreateSupplyMovementInput`, which omits the two snapshot columns and requires `performed_by` (to be taken from the authenticated user's token, never from the request body).
...
- **Appointments**: `status` is an enum (`scheduled`, `checked_in`, `completed`, `cancelled`, `no_show`); `is_walk_in` and `is_reward_redemption` are explicit boolean flags; `barber_id` is nullable specifically to allow an unassigned walk-in until check-in, enforced by a `CHECK` constraint (`is_walk_in = true OR barber_id IS NOT NULL`). Appointments for **unregistered clients** are supported: `client_id` is nullable (`ON DELETE SET NULL`) and `guest_client_name` holds the only data captured for a guest. Because `client_id` is also `NULL` for a registered client whose account was deleted, an explicit `is_registered_client boolean NOT NULL` (no default, set by the backend at creation, never changed) distinguishes the two, instead of inferring it from a null. The `chk_appointments_client_or_guest` constraint enforces: registered → no guest name (and `client_id` may be `NULL` only after deletion); guest → `client_id` null and a guest name required. Backend queries must use `LEFT JOIN` to clients/users for appointments, and the reminder pipeline must skip rows with `client_id IS NULL`. `appointment_services` snapshots `price_at_booking`/`duration_minutes_at_booking` so past revenue isn't distorted by later catalog price changes.
...
Still open:
- Implementing the barber-deletion rule (block while pending appointments exist, allow after they're cancelled). Two parts: the application must check for pending appointments (statuses `scheduled`/`checked_in`) and reject the delete with a clear error, and the schema must let the delete succeed afterwards. The latter means changing `appointments.barber_id` from `RESTRICT` to `SET NULL` (or similar) for historical rows, which in turn requires relaxing `chk_appointments_barber_required` (`is_walk_in = true OR barber_id IS NOT NULL`), since a `SET NULL` on a non-walk-in appointment would violate it and abort the delete. `availability_slots.barber_id` already cascades. Whether the pending-appointment check is done in the application or enforced by a `BEFORE DELETE` trigger is also undecided.
- Whether `supply_stock` should be updated by a trigger from `supply_movements` rather than by the application in a transaction.
...
Not implemented. Modeled in the schema (`notifications_log`, with a free-text `trigger_type` since reminder timing isn't finalized; `client_id` is `NOT NULL`, so appointments for unregistered clients can't have notifications and the scheduler must skip them), but there is no scheduler, no WhatsApp/SES integration, and no code that writes to this table yet. Design considerations, unchanged from the original plan:
...
**Booking an appointment** (staff booking for an unregistered client follows the same flow, with `is_registered_client = false` and only `guest_client_name` supplied, and no reminder job is enqueued): Client app → Backend `appointment` controller/service checks barber availability against `availability_slots` and existing bookings → writes `appointments` row → enqueues a reminder job (e.g. "send 24h before, send 2h before") → returns confirmation to client.
