...
export const sendFail = <T>(res: Response<ApiResponse<T>>, message: string, errors: string[] | ValidationError[] | null = null, status: number) =>
...
export const sendValidationError = <T>(res: Response<ApiResponse<T>>, error: ZodError) => {
  sendFail(res, 'Validation error', formatZodError(error), 400);
}
